...
Ти — senior solution architect + system analyst із досвідом у:
- enterprise-системах
- комп’ютерному баченні (CV)
- OCR
- ML / AI pipelines
- edge AI (NVIDIA Dev Kit)
- backend-архітектурі

Твоя задача — **підготувати повне технічне завдання (ТЗ)** для R&D-проєкту.

---

# Контекст проєкту

Проєкт — **дослідницьке технодемо (не production)** для готельного бізнесу.

Мета:
- отримувати фото гостей і їхніх документів
- зберігати ці дані в базі
- виконувати OCR документів різних країн
- підготувати систему до розпізнавання облич
- у перспективі інтегрувати NVIDIA Dev Kit + камеру

---

# Джерела даних

Система повинна підтримувати **2 канали отримання даних**:

## 1. Внутрішня пошта (Email Ingestion)
- Linux-based внутрішній mail server
- без доменів, DNS, MX
- використовується як технічний канал доставки вкладень
- автоматичний парсинг вкладень (image / pdf)

## 2. Web-застосунок для ресепшну
- Web app (React)
- доступ до камери браузера
- форма введення даних гостя
- завантаження або фотографування документів
...
- OCR документів:
  - закордонні паспорти
  - ID-картки різних країн (ЄС, Україна, інші)
- Витяг структурованих даних:
  - ім’я
  - прізвище
  - номер документа
  - країна (якщо можливо)
- Використання **cloud OCR API** (на етапі R&D)
...
- Зберігання:
  ...
  - OCR результатів
  - статусів обробки
- Підготовка до face embeddings (vector representation)
...
## Поточний етап
- OCR через cloud API
- Face detection та підготовка embeddings

## Майбутній етап
- Інтеграція NVIDIA Dev Kit
- Камера (статична, ресепшн)
- Дослідження моделей:
  - V-JEPA
  - аналоги
- Аналіз можливостей:
  ...
  - face recognition
  - tracking
  - поведінкові гіпотези

---

# Вимоги до результату (ТЗ)

Підготуй **детальне технічне завдання у Markdown**, яке містить:

## 1. Загальний опис системи
- мета
- обмеження
- R&D характер проєкту
...
## 5. Діаграми (у Markdown)
Побудуй:
- system architecture diagram
- data flow diagram
- AI pipeline diagram
- future NVIDIA Dev Kit integration diagram

(Використовуй Mermaid або ASCII-діаграми)

## 6. Етапи реалізації (Roadmap)
- фази
- залежності
- що можна робити без заліза
- що потребує dev kit
...
```
Технічне завдання створено у файлі index.md. Документ містить:

1. **Загальний опис** — мета, scope, обмеження R&D проєкту
2. **Архітектура** — високорівнева, backend, ingestion/processing pipelines
3. **Технологічний стек** — повний перелік технологій (Python/FastAPI, React, OCR APIs, ML/CV бібліотеки, NVIDIA stack)
4. **Детальний опис 6 модулів** — Mail Ingestion, Web Ingestion, OCR Service, Data Normalization, Face Processing, Database
5. **5 діаграм у Mermaid/ASCII** — System Architecture, Data Flow, AI Pipeline, NVIDIA Integration, Component Interaction
6. **Roadmap з 4 фазами** — з чітким розподілом, що можна робити без Dev Kit
7. **Ризики та обмеження** — OCR якість, освітлення, hardware залежності
8. **Deliverables** — по кожній фазі та фінальні

Документ готовий для review та подальшої роботи.

Made changes.

Eugene-pas: На основі створеного ТЗ сформуй список завдань які мені потрібно виконати для реалізації проекта

GitHub Copilot: На основі ТЗ формую структурований список завдань для реалізації проєкту:
